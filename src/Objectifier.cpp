...
void Objectifier::run() {
    ...
    this->lines.reserve((this->info->x_lines.size() + this->info->y_lines.size()) * 2);
    ...
    for (const MapProcessing::AxisLineSegment& line : this->info->x_lines) {
        if (line.range_start != line.range_end) {
            // if (line.range_end - 8 > line.range_start + 4) {
            this->lines.emplace_back(line.axis + 9, line.range_start - 8, line.axis + 9, line.range_end + 4);
            this->lines.emplace_back(line.axis - 9, line.range_start - 8, line.axis - 9, line.range_end + 4);
            // }

            // this->lines.emplace_back(line.axis + 9, line.range_start - 8, line.axis - 9, line.range_start - 8);
            this->lines.emplace_back(line.axis + 9, line.range_start - 8, line.axis - 9, line.range_start - 8);

            this->lines.emplace_back(line.axis + 9, line.range_end + 4, line.axis - 9, line.range_end + 4);
            // this->lines.emplace_back(line.axis + 9, line.range_end + 4, line.axis - 9, line.range_end + 4);
        }
    }
    ...
    for (const MapProcessing::AxisLineSegment& line : this->info->y_lines) {
        if (line.range_start != line.range_end) {
            // if (line.range_end - 9 > line.range_start + 9) {
                this->lines.emplace_back(line.range_start - 9, line.axis + 4, line.range_end + 9, line.axis + 4);
                this->lines.emplace_back(line.range_start - 9, line.axis - 8, line.range_end + 9, line.axis - 8);
            // }
            // this->lines.emplace_back(line.range_start, line.axis, line.range_end, line.axis);

            // this->lines.emplace_back(line.range_start + 9, line.axis - 8, line.range_start + 9, line.axis + 4);
            this->lines.emplace_back(line.range_start - 9, line.axis - 8, line.range_start - 9, line.axis + 4);
            
            this->lines.emplace_back(line.range_end + 9, line.axis - 8, line.range_end + 9, line.axis + 4);
            // this->lines.emplace_back(line.range_end - 9, line.axis - 8, line.range_end - 9, line.axis + 4);
        }
    }
